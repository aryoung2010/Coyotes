...
# Reading samples
sample <- read.csv("cleaned_data.csv", stringsAsFactors = FALSE)
sample <- sample[1:812,]
colnames(sample)[13] <- "zip"
sample$zip2[sample$zip < 10000 & is.na(sample$zip) == F] <- sprintf("%05d", sample$zip[sample$zip < 10000 & is.na(sample$zip) == F])
